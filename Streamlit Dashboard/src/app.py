...
if option == 'Random Password':
    length = st.slider("Password Length", min_value=5, max_value=50, value=8)
    include_numbers = st.toggle("Include Numbers")
    include_symbols = st.toggle("Include Symbols")

    generator = RandomPasswordGenerator(length=length, include_numbers=include_numbers, include_symbols=include_symbols)
    ...
    length = st.slider("Pin Code Length", min_value=2, max_value=8, value=4)
